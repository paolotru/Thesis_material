...
RoutingProtocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::aodv::RoutingProtocol")
            ...
            .AddAttribute ("EnableBHatk", "Indicates whether a BlackHole attack is enabled or not",
                            BooleanValue (false),
                            ...
                            ErrorCallback ecb)
{
    ...
    //mod
    //Dropping packet instead of forwarding if in blackhole simulation
    if(EnableBHatk){
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (!m_routingTable.LookupRoute(origin, toOrigin)){
        ...
        Ptr<NetDevice> dev;
        //mod 
        if(EnableWrmAttack && (src==FirstEndOfWormTunnel || src==FirstEndWifiWormTunnel)){
            std::cout << "Wormhole attack occuring, redirecting to second end of tunnel... \n";
      	    dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(SecondEndOfWormTunnel));
      		receiver=SecondEndOfWormTunnel;
      	}
        else if(EnableWrmAttack && (src==SecondEndOfWormTunnel || src==SecondEndWifiWormTunnel)){
            std::cout << "Wormhole attack occuring, redirecting to first end of tunnel... \n";
    ...
    else
    {
        ...
        // if(EnableWrmAttack){
        //     if(src==FirstEndOfWormTunnel){
        //         toOrigin.SetOutputDevice(m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(SecondEndOfWormTunnel)));
        //         toOrigin.SetInterface(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(SecondEndOfWormTunnel), 0));
        //     }else if(src==SecondEndOfWormTunnel){ 
        //         toOrigin.SetOutputDevice(m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(FirstEndOfWormTunnel)));
        //         toOrigin.SetInterface(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(FirstEndOfWormTunnel), 0));
        //     }
        // }else{
    ...
    if (EnableBHatk || EnableWrmAttack || m_routingTable.LookupRoute(dst, toDst))
    {
        /*
        ...
        //mod  
        if (EnableBHatk || EnableWrmAttack || ((rreqHeader.GetUnknownSeqno() ||
             (int32_t(toDst.GetSeqNo()) - int32_t(rreqHeader.GetDstSeqno()) >= 0)) &&
            toDst.GetValidSeqNo()))
        {
            if(EnableBHatk || EnableWrmAttack || (!rreqHeader.GetDestinationOnly() && toDst.GetFlag() == VALID))
            {
                m_routingTable.LookupRoute(origin, toOrigin);

                //mod
                if(EnableBHatk || EnableWrmAttack){
                    Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress (receiver));
                                             ...
                                             bool gratRep)
{
    ...
    //mod 
    /*  Attracting nodes to send through malicious node  */
